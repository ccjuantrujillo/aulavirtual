...
class Cliente extends Persona
{
    ...
    public function listar($j=0){  
        ...
        $registros = count($this->cliente_model->listar($filter,$filter_not));
        $clientes  = $this->cliente_model->listar($filter,$filter_not,$this->configuracion['per_page'],$j);
        $item      = 1;
        $lista     = array();
        if(count($clientes)>0){
    ...
    public function grabar(){  
        ...
        elseif($accion == "e"){            
            $this->cliente_model->modificar($codigo_padre,$data); 
        }
    }
    ...
    public function eliminar(){
        ...
        if(count($ordenes) == 0){
            ...
            $filter = new stdClass();
            $filter->cliente = $codigo;
            $cliente = $this->cliente_model->obtener($filter);
            $persona = $cliente->PERSP_Codigo;
            $this->cliente_model->eliminar($codigo);
